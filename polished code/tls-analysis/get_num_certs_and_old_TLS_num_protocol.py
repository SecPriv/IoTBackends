...
result_json_file_name = protocol + "_result.json"
revoked_certs_file_name = protocol + "_revoked_certs.json"
...
def evaluate_certs (endpoint, path):
    global num_of_endpoints_with_revocation_list, num_certificates
    certs_json = endpoint["certs"]
    has_revocation_list = False
    num_certificates += len(certs_json)
...
evaluate_result_json(base_folder)

print("Total number of certificates", num_certificates)
print("Total TLS < 1.1", count_old_tls_version)
